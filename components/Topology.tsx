...
const Topology: React.FC = () => {
  ...
  const scale = useMemo(() => {
    ...
    // Don't scale up too much, but scale down for mobile
    return Math.min(Math.max(wScale, 0.3), 1.1);
  }, [dimensions]);
  ...
  return (
    ...
    >
      ...
      {/* DRAGGABLE TERMINAL */}
      <div className="absolute bottom-4 left-4 md:bottom-8 md:left-8 z-50 w-full max-w-[350px] md:max-w-[420px] pointer-events-none">
        <motion.div 
          drag
          dragConstraints={{
            top: -dimensions.h - 100, 
            left: -300,
            right: dimensions.w - 100,
            bottom: 200
          }}
          dragMomentum={false}
          dragElastic={0.1}
